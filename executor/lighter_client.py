...
OPTIMIZACIÓN: Uso de índices dinámicos (Index 3 corregido).
"""
...
def load_lighter_pk() -> str | None:
    ...
    try:
        ...
        return secrets.get("LIGHTER_API_KEY") or secrets.get("LIGHTER_PRIVATE_KEY")
    except Exception as e:
        logger.error("Error cargando PK dinamicamente: %s", e)
        return os.environ.get("LIGHTER_API_KEY") or os.environ.get("LIGHTER_PRIVATE_KEY")
...
class LighterClient:
    def __init__(self, dry_run: bool = False):
        ...
        if not dry_run:
            try:
                ...
                logger.info("Lighter conectado — Account: %s | Index: %s", config.LIGHTER_ACCOUNT_INDEX, config.LIGHTER_API_INDEX)
    ...
    async def get_signer(self):
        ...
        return lighter.SignerClient(
            url="https://mainnet.zklighter.elliot.ai",
            api_private_keys={config.LIGHTER_API_INDEX: self._pk},
    ...
    async def get_position(self, coin: str) -> dict | None:
        try:
            ...
            if not resp or not resp.accounts: return None
    ...
    async def get_open_orders(self, signer) -> list:
        try:
            from src import config
            auth_data = signer.create_auth_token_with_expiry(deadline=600, api_key_index=config.LIGHTER_API_INDEX)
    ...
    async def cancel_all_orders(self, signer, nonce_start: int) -> int:
        ...
        for o in orders:
            ...
            if oid:
                ...
                await signer.cancel_order(1, int(oid), nonce, config.LIGHTER_API_INDEX)
    ...
    async def market_order(self, signer, coin: str, is_buy: bool, sz: float, nonce: int) -> dict:
        ...
        try:
            ...
            tx, tx_hash, err = await signer.create_market_order(1, 0, sz_int, px_int, not is_buy, False, nonce_real, config.LIGHTER_API_INDEX)
    ...
    async def get_ticker(self, coin: str) -> dict:
        try:
            details = await self._order_api.order_book_details(market_id=1)
            return {"last": float(details.order_book_details[0].last_trade_price)}
        except: return {"last": 0.0}
